...
function Task_00000630(npcId)
	...
	if task:GetTaskAcceptNpc(630) == npcId and Task_Accept_00000630 () then
		...
		action.m_ActionMsg = task_msg_001299;
	elseif task:GetTaskSubmitNpc(630) == npcId then
		if Task_Submit_00000630() then
			...
			action.m_ActionMsg = task_msg_001300;
		elseif task:HasAcceptedTask(630) then
			action.m_ActionType = 0x0001;
			action.m_ActionID = 630
			action.m_ActionToken = 0;
			action.m_ActionStep = 0;
			action.m_ActionMsg = task_msg_001301;
		end
	end
	return action;
end
...
function Task_00000630_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = task_msg_001302..GetPlayerName(GetPlayer())..task_msg_001303;
	action.m_ActionMsg = task_msg_001304;
	return action;
end
...
function Task_00000630_step_10()
	...
	action.m_ActionToken = 3;
	action.m_ActionStep = 0;
	action.m_NpcMsg = GetPlayerName(GetPlayer())..task_msg_001305;
	action.m_ActionMsg = "";
	return action;
end
