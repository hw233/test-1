...
function Task_00000513_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = "ç™½äº‘å¤§å¸ˆä¹ƒæ˜¯æˆ‘é“ä¸­äººï¼Œå‰‘æœ¯é€šç¥ï¼Œä½ åº”è¯¥å»æ‹œè®¿ä¸€ä¸‹å¥¹ã€‚";
...
function Task_00000513_step_10()
	...
	action.m_ActionToken = 3;
	action.m_ActionStep = 0;
	action.m_NpcMsg = "å¤šè°¢å°å‹çš„å¥½æ„ã€‚";
	action.m_ActionMsg = "";
	return action;
end
...
function Task_00000513_submit(itemId, itemNum)
	...
	if task:CanDayTaskSubmit(513) then
		if DayTaskAward(0) then
			task:DayTaskSubmit(513);
			return true;
		end
	end
