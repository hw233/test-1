...
function Task_00000629(npcId)
	...
	if task:GetTaskAcceptNpc(629) == npcId and Task_Accept_00000629 () then
		...
		action.m_ActionMsg = task_msg_001292;
	elseif task:GetTaskSubmitNpc(629) == npcId then
		if Task_Submit_00000629() then
			...
			action.m_ActionMsg = task_msg_001293;
		elseif task:HasAcceptedTask(629) then
			action.m_ActionType = 0x0001;
			action.m_ActionID = 629
			action.m_ActionToken = 0;
			action.m_ActionStep = 0;
			action.m_ActionMsg = task_msg_001294;
		end
	end
	return action;
end
...
function Task_00000629_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = task_msg_001295..GetPlayerName(GetPlayer())..task_msg_001296;
	action.m_ActionMsg = task_msg_001297;
	return action;
end
...
function Task_00000629_step_10()
	...
	action.m_ActionToken = 3;
	action.m_ActionStep = 0;
	action.m_NpcMsg = GetPlayerName(GetPlayer())..task_msg_001298;
	action.m_ActionMsg = "";
	return action;
end
