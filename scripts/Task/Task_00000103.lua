...
function Task_00000103_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = "æˆéƒ½é™„è¿‘æœ‰åº§æ…ˆäº‘ç¦…å¸ˆï¼Œå¯ºå†…ä¸»æŒæ™ºé€šæœ¬å®˜è¿˜è§è¿‡ï¼Œä¸€ç›´éƒ½å®‰åˆ†å®ˆå·²ï¼Œå¯æ˜¯æœ€è¿‘å¯ºå†…æ¥äº†ä¸å°‘äº”æ¹–å››æµ·çš„å¼ºäººï¼Œåœ¨æˆéƒ½ä¸€å¸¦ä¸ºæ¶ï¼Œç”šè‡³è¿å¯ºå†…çš„å’Œå°šéƒ½å¼€å§‹ä½œå¥¸çŠ¯ç§‘ï¼Œ<name>ä½ è¿™å»å°†è¿™äº›ä¸ªå¦–åƒ§å‰¿ç­ã€‚";
...
function Task_00000103_submit(itemId, itemNum)
	...
	if task:CanDayTaskSubmit(103) then
		if DayTaskAward(0) then
			task:DayTaskSubmit(103);
			return true;
		end
	end
