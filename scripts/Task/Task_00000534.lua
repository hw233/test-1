...
function Task_00000534(npcId)
	...
	if task:GetTaskAcceptNpc(534) == npcId and Task_Accept_00000534 () then
		...
		action.m_ActionMsg = task_msg_001424;
	elseif task:GetTaskSubmitNpc(534) == npcId then
		if Task_Submit_00000534() then
			...
			action.m_ActionMsg = task_msg_001425;
		elseif task:HasAcceptedTask(534) then
			action.m_ActionType = 0x0001;
			action.m_ActionID = 534
			action.m_ActionToken = 0;
			action.m_ActionStep = 0;
			action.m_ActionMsg = task_msg_001426;
		end
	end
	return action;
end
...
function Task_00000534_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = task_msg_001427..GetPlayerName(GetPlayer())..task_msg_001428;
	action.m_ActionMsg = task_msg_001429;
	return action;
end
...
function Task_00000534_step_10()
	...
	action.m_ActionToken = 3;
	action.m_ActionStep = 0;
	action.m_NpcMsg = GetPlayerName(GetPlayer())..task_msg_001430;
	action.m_ActionMsg = "";
	return action;
end
