...
function Task_00000512_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = "åœ¨æˆéƒ½æœ‰ä½éšå±…å¤šå¹´çš„å‰‘ä¾ ï¼Œå«æ²ˆæµªï¼Œæ˜”å¹´ä¸ºå¸ˆæ›¾å’Œä»–ä¸€èµ·å‡ºé—¨å†ç»ƒï¼Œä½ å»æ‹œè®¿ä¸€ä¸‹ä»–å§ï¼Œ";
...
function Task_00000512_submit(itemId, itemNum)
	...
	if task:CanDayTaskSubmit(512) then
		if DayTaskAward(0) then
			task:DayTaskSubmit(512);
			return true;
		end
	end
