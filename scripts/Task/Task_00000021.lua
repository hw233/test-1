...
function Task_00000021(npcId)
	...
	if task:GetTaskAcceptNpc(21) == npcId and Task_Accept_00000021 () then
		...
		action.m_ActionMsg = "å¤šè‡‚ç†Šæ¯›å¤ª";
...
function Task_00000021_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = GetPlayerName(GetPlayer()).."åˆšåˆšæˆ‘çœ‹è§å¤šè‡‚ç†Šæ¯›å¤ªé¬¼é¬¼ç¥Ÿç¥Ÿå»äº†é­é’å®¶çš„èŒ…èˆï¼Œè™½ç„¶ä¸æ¸…æ¥šä»–å»åšä»€ä¹ˆï¼Œä¸è¿‡è¿™ç§å¦–äººå»è‚¯å®šä¸æ˜¯ä»€ä¹ˆå¥½äº‹ï¼Œå°‘ä¾ ä¸å¦‚è·Ÿå»çœ‹çœ‹ã€‚";
	action.m_ActionMsg = "æˆ‘è¿™å°±è¿‡å»ã€‚";
	return action;
end
...
function Task_00000021_step_10()
	...
	action.m_ActionToken = 3;
	action.m_ActionStep = 0;
	action.m_NpcMsg = "å°‘ä¾ æ•‘å‘½ï¼Œè¿™ä¸ªæ¯›å’Œå°šæ¬²éç¤¼äºæˆ‘â€¦â€¦";
	action.m_ActionMsg = "";
	return action;
end
