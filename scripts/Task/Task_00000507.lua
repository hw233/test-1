...
function Task_00000507(npcId)
	...
	elseif task:GetTaskSubmitNpc(507) == npcId then
		if Task_Submit_00000507() then
			...
			action.m_ActionMsg = task_msg_672;
		elseif task:HasAcceptedTask(507) then
			action.m_ActionType = 0x0001;
			action.m_ActionID = 507
			action.m_ActionToken = 0;
			action.m_ActionStep = 0;
			action.m_ActionMsg = task_msg_673;
		end
	end
	return action;
end
...
function Task_00000507_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = task_msg_674..GetPlayerName(GetPlayer())..task_msg_675;
	action.m_ActionMsg = task_msg_676;
	return action;
end
...
function Task_00000507_step_10()
	...
	action.m_ActionToken = 3;
	action.m_ActionStep = 0;
	action.m_NpcMsg = GetPlayerName(GetPlayer())..task_msg_677;
	action.m_ActionMsg = "";
	return action;
end
