...
function Task_00000508(npcId)
	...
	if task:GetTaskAcceptNpc(508) == npcId and Task_Accept_00000508 () then
		...
		action.m_ActionMsg = task_msg_646;
	elseif task:GetTaskSubmitNpc(508) == npcId then
		if Task_Submit_00000508() then
			...
			action.m_ActionMsg = task_msg_647;
		elseif task:HasAcceptedTask(508) then
			action.m_ActionType = 0x0001;
			action.m_ActionID = 508
			action.m_ActionToken = 0;
			action.m_ActionStep = 0;
			action.m_ActionMsg = task_msg_648;
		end
	end
	return action;
end
...
function Task_00000508_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = task_msg_649..GetPlayerName(GetPlayer())..task_msg_650;
	action.m_ActionMsg = task_msg_651;
	return action;
end
...
function Task_00000508_step_10()
	...
	action.m_ActionToken = 3;
	action.m_ActionStep = 0;
	action.m_NpcMsg = GetPlayerName(GetPlayer())..task_msg_652;
	action.m_ActionMsg = "";
	return action;
end
