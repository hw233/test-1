...
function Task_00000508_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = "æˆéƒ½å‰æ®µæ—¶é—´æ²»å®‰çœŸå¾—å¾ˆæˆé—®é¢˜ï¼Œæœ‰ä¸€äº›æ…ˆäº‘å¯ºå‡ºæ¥çš„å¦–äººåœ¨é™„è¿‘ä¸ºæ¶ï¼Œç¥¸å®³ç™¾å§“ï¼Œå®åœ¨æ˜¯æˆ‘ä»¬å‰‘ä¾ é‡Œçš„è´¥ç±»ï¼Œ"..GetPlayerName(GetPlayer()).."ä½ é€Ÿé€Ÿå»å°†è¿™äº›æ¶äººé™¤æ‰ã€‚";
...
function Task_00000508_submit(itemId, itemNum)
	...
	if task:CanDayTaskSubmit(508) then
		if DayTaskAward(0) then
			task:DayTaskSubmit(508);
			return true;
		end
	end
