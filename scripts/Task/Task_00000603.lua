...
function Task_00000603(npcId)
	...
	if task:GetTaskAcceptNpc(603) == npcId and Task_Accept_00000603 () then
		...
		action.m_ActionMsg = task_msg_874;
	elseif task:GetTaskSubmitNpc(603) == npcId then
		if Task_Submit_00000603() then
			...
			action.m_ActionMsg = task_msg_875;
		elseif task:HasAcceptedTask(603) then
			action.m_ActionType = 0x0001;
			action.m_ActionID = 603
			action.m_ActionToken = 0;
			action.m_ActionStep = 0;
			action.m_ActionMsg = task_msg_876;
		end
	end
	return action;
end
...
function Task_00000603_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = task_msg_877..GetPlayerName(GetPlayer())..task_msg_878;
	action.m_ActionMsg = task_msg_879;
	return action;
end
...
function Task_00000603_step_10()
	...
	action.m_ActionToken = 3;
	action.m_ActionStep = 0;
	action.m_NpcMsg = GetPlayerName(GetPlayer())..task_msg_880;
	action.m_ActionMsg = "";
	return action;
end
