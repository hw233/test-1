...
function Task_00000607(npcId)
	...
	if task:GetTaskAcceptNpc(607) == npcId and Task_Accept_00000607 () then
		...
		action.m_ActionMsg = task_msg_902;
	elseif task:GetTaskSubmitNpc(607) == npcId then
		if Task_Submit_00000607() then
			...
			action.m_ActionMsg = task_msg_903;
		elseif task:HasAcceptedTask(607) then
			action.m_ActionType = 0x0001;
			action.m_ActionID = 607
			action.m_ActionToken = 0;
			action.m_ActionStep = 0;
			action.m_ActionMsg = task_msg_904;
		end
	end
	return action;
end
...
function Task_00000607_step_01()
	...
	action.m_ActionStep = 0;
	action.m_NpcMsg = task_msg_905..GetPlayerName(GetPlayer())..task_msg_906;
	action.m_ActionMsg = task_msg_907;
	return action;
end
...
function Task_00000607_step_10()
	...
	action.m_ActionToken = 3;
	action.m_ActionStep = 0;
	action.m_NpcMsg = GetPlayerName(GetPlayer())..task_msg_908;
	action.m_ActionMsg = "";
	return action;
end
