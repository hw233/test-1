function Collect_00004139()
	local player = GetPlayer();
	local action = ActionTable:Instance();
	if player:GetTaskMgr():HasAcceptedTask(60802) then
...
function Collect_00004139_Item_Action()
	local player = GetPlayer();
	local task = player:GetTaskMgr();
	if task:HasAcceptedTask(60802) then
		if not player:GetPackage():ExistItem(15009) and player:GetPackage():AddItem(15009,1,1) then
			task:AddTaskStep(60802);
			return true;
		end
	end
	return false;
