...
if [ $# -ge 2 -a "$2" == "fb" ]; then
    make release_fb
    if [ $# -ge 3 -a "$3" == "s" ]; then
        tar cvfz $R$file --exclude=sql/conf.sh bin/Release/Server.ASSS conf/udplog.xml conf/udplogfb.xml scripts/ sql/
    else
        cp -rf sql sql.bak
        cp -rf scripts scripts.bak
        ./toft.sh
        tar cvfz $R"FB."$file --exclude=sql/conf.sh bin/Release/Server.ASSS conf/udplog.xml conf/udplogfb.xml scripts/ sql/
        rm -rf sql scripts
        mv sql.bak sql
