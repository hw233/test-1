...
int query_usr_threeyears_data_req(JsonHead* head, json_t* body, json_t* retbody, std::string& err)
{
    if (!head || !body || !retbody)
        return EUNKNOW;
    ...
    memset(buf, 0x00, sizeof(buf));
    snprintf(buf, sizeof(buf), "%u", player->GetMarriageInfo()->yuyueTime);
    json_insert_pair_into_object(retbody, "szMarryTime", json_new_string(buf));

    memset(buf, 0x00, sizeof(buf));
    snprintf(buf, sizeof(buf), "%u", player->getClanJoinTime());
    json_insert_pair_into_object(retbody, "szLastJoinTime", json_new_string(buf));
    ...
    memset(buf, 0x00, sizeof(buf));
    snprintf(buf, sizeof(buf), "%u", petTime);
    json_insert_pair_into_object(retbody, "szGetPetTime", json_new_string(buf));

    memset(buf, 0x00, sizeof(buf));
    snprintf(buf, sizeof(buf), "%u", fighterTime);
    json_insert_pair_into_object(retbody, "szSummonTime", json_new_string(buf));

    memset(buf, 0x00, sizeof(buf));
    snprintf(buf, sizeof(buf), "%u", fighterCount);
    json_insert_pair_into_object(retbody, "szSummonNum", json_new_string(buf));
...
void jsonParser2(void * buf, int len, Stream& st)
{
    ...
    switch (head.cmd)
    {
        ...
        case 119:
            ...
            break;
        case 121:
            ret = query_usr_threeyears_data_req(&head, body, retbody, err);
