...
void GMHandler::OnAddjgy(GObject::Player * player, std::vector<std::string>& args)
{
    if(player->GetLev() < 70)
        return;
	...
	if(args.size() == 1)
	{
		UInt32 jiguanyuId = atoi(args[0].c_str());

        player->GetMoFang()->addjiguanyu(jiguanyuId);
	}
}

void GMHandler::OnGetKey(GObject::Player * player, std::vector<std::string>& args)
{
    if(player->GetLev() < 70)
        return;
	...
	if(args.size() == 1)
	{
		UInt16 keyId = atoi(args[0].c_str());

        player->GetMoFang()->checkKey(keyId, 12);
	}
}
...
#pragma pack()
    ...
    switch(type)
    {
        ...
        case 37:
            ...
            GVAR.SetVar(GObject::GVAR_NATIONAL_DAY_END, TimeUtil::SharpDayT(2));
            break;
    }
}
...
void GMHandler::OnAddCardExp(GObject::Player *player, std::vector<std::string>& args)
{
    ...
    player->GetCollectCard()->GMAddExp(exp);   
