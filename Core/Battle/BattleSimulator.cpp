...
UInt32 BattleSimulator::doSpiritAttack(BattleFighter * bf, BattleFighter* bo, float atk, bool& pr, bool& cs, bool& first)
{
    ...
    if(!defend100 && (target_stun > 0 || (!enterEvade && bf->calcHit(bo, NULL) && !bo->getMoEvade100())))
    {
        ...
        else
        {
            ...
            dmg = _formula->calcDamage(atk, def, bf->getLevel(), toughFactor, atkreduce, _formula->calcAttackPierce(bf));
...

           {
            ...
            else
            {
                ...
                if(magatk)
                {
                    ...
                    magdmg = _formula->calcDamage(factor * magatk, magdef, bf->getLevel(), toughFactor, magatkreduce, _formula->calcAttackPierce(bf));
                ...
                if(atk)
                {
                    ...
                    float atkreduce = getBFAtkReduce(area_target);
                    dmg = _formula->calcDamage(factor * atk, def, bf->getLevel(), toughFactor, atkreduce, _formula->calcAttackPierce(bf));
        ...
        if(can_counter && counter_deny >= 0 && _winner == 0 && !_isBody[side][pos] && area_target_obj->getHP() > 0 && bf->getHP() > 0)
        {
            ...
            if(counter100 || target_fighter->calcCounter(bf, skill))
            {
                if(counter100 || target_fighter->calcHit(bf, NULL))
                {
                    ...
                    UInt32 dmg2 = _formula->calcDamage(atk * factor, def, target_fighter->getLevel(), toughFactor, atkreduce, _formula->calcAttackPierce(target_fighter));
                    ...
                    target_fighter->setCounterCnt(target_fighter->getCounterCnt() + 1);
                ...
                || (skillId == 153) || (skillId == 154) || (skillId == 155) || (skillId == 161) || skillId == 556 || skillId == 557)
        {
            ...
            if(bo != NULL && bo->getHP() != 0 && bo->isChar())
            {
                ...
                if((rate * factor) > _rnd(10000))
                {
                    if(skillId == 150 || skillId == 153 || skillId == 556 || skillId == 557)
                    {
                        ...
                        UInt32 dmg = _formula->calcDamage(factor*getBFMagAtk(bf)*0.35f, magdef, bf->getLevel(), 1, 0, _formula->calcAttackPierce(bf));
                    ...
                    else if(skillId == 152 || skillId == 155)
                    {
                        ...
                        UInt32 dmg = _formula->calcDamage(factor*getBFAttack(bf)*0.35f, def, bf->getLevel(), 1, 0, _formula->calcAttackPierce(bf));
            ...
            for(int i = 0; i < apcnt; ++ i)
            {
                ...
                if(rate > _rnd(10000))
                {
                    ...
                    {
                        UInt32 magdef = getBFMagDefend(bo);
                        UInt32 dmg = _formula->calcDamage(ap[i].factor*getBFMagAtk(bf)*0.35f, magdef, bf->getLevel(), 1, 0, _formula->calcAttackPierce(bf));
                        bo->setAuraBleed(dmg, skill->last, 5);
                        appendDefStatus(e_Bleed1, 0, bo);
                        //calcAbnormalTypeCnt(bo);
                    ...
                    {
                        UInt32 def = getBFDefend(bo);
                        UInt32 dmg = _formula->calcDamage(ap[i].factor*getBFAttack(bf)*0.35f, def, bf->getLevel(), 1, 0, _formula->calcAttackPierce(bf));
                        bo->setStunBleed(dmg, skill->last, 5);
                        appendDefStatus(e_Bleed3, 0, bo);
                        if(ss)
                        {
...
bool BattleSimulator::doSkillStatus2(BattleFighter* bf, const GData::SkillBase* skill, int target_side, int target_pos, int cnt)
{
    ...
    if(skill->effect->defP || skill->effect->def)
    {
        ...
        if(value > 0 && bf->getSide() != target_side)
        {
            ...
            if(skill->cond == GData::SKILL_ENTER_LINGSHI)
                setStatusChange(bf, bf->getSide(), bf->getPos(), 1, skill, e_stDef, value, bf->getNewModeLast(), bf->getSide() != 0);
            else
        ...
        {
            if(skill->cond == GData::SKILL_ENTER_LINGSHI)
                setStatusChange(bf, target_side, bo == NULL ? 0 : bo->getPos(), cnt, skill, e_stDef, value, bf->getNewModeLast(), target_side != 0);
            else
    ...
    if(skill->effect->magdefP || skill->effect->magdef)
    {
        ...
        if(value > 0 && bf->getSide() != target_side)
        {
            ...
            if(skill->cond == GData::SKILL_ENTER_LINGSHI)
                setStatusChange(bf, bf->getSide(), bf->getPos(), 1, skill, e_stMagDef, value, bf->getNewModeLast(), bf->getSide() != 0);
            else
        ...
        {
            if(skill->cond == GData::SKILL_ENTER_LINGSHI)
                setStatusChange(bf, target_side, bo == NULL ? 0 : bo->getPos(), cnt, skill, e_stMagDef, value, bf->getNewModeLast(), target_side != 0);
            else
    ...
    if(skill->effect->evade)
    {
        ...
        {
            if(skill->cond == GData::SKILL_ENTER_LINGSHI)
                setStatusChange(bf, bf->getSide(), bf->getPos(), 1, skill, e_stEvade, value, bf->getNewModeLast(), bf->getSide() != 0);
            else
        ...
        {
            if(skill->cond == GData::SKILL_ENTER_LINGSHI)
                setStatusChange(bf, target_side, bo == NULL ? 0 : bo->getPos(), cnt, skill, e_stEvade, value, bf->getNewModeLast(), target_side != 0);
            else
    ...
    if(skill->effect->critical)
    {
        ...
        {
            if(skill->cond == GData::SKILL_ENTER_LINGSHI)
                setStatusChange(bf, bf->getSide(), bf->getPos(), 1, skill, e_stCritical, value, bf->getNewModeLast(), bf->getSide() != 0);
            else
        ...
        {
            if(skill->cond == GData::SKILL_ENTER_LINGSHI)
                setStatusChange(bf, target_side, bo == NULL ? 0 : bo->getPos(), cnt, skill, e_stCritical, value, bf->getNewModeLast(), target_side != 0);
            else
    ...
    for(UInt8 side = 0; side < 2; side++)
    {
        for(UInt8 i = 0; i < 25; i++)
        {
            ...
            if(bo->getTyslSSFactor() < 0.001f)
                continue;
            if(bo->getTyslSSCnt2() == 0)
                continue;
            UInt8 originCnt = bo->getTyslSSCnt() / 5;
            bo->setTyslSSCnt(bo->getTyslSSCnt() + bo->getTyslSSCnt2());
...
UInt32 BattleSimulator::GetBleedDmg(BattleFighter* bf, BattleFighter* bo, float nfactor)
{
    ...
    nRetDmg = _formula->calcDamage(nfactor*attack, def, bf->getLevel(), 1.0f, reduce, _formula->calcAttackPierce(bf)); // 坚韧是在破击的时候起作用的，这里暴击、破击都不算
    return nRetDmg;
}
        ...
        if(dmgCnt > 0)
        {
            ...
            for(int i = 0; i < dmgCnt; ++ i)
            {
                ...
                if(rnd_bf)
                {
                    ...
                    doSSMakeDamage(bf, rnd_bf, factor);
...
void BattleSimulator::doItemWu_Dmg(BattleFighter* bf, BattleFighter* bo, float value, UInt8 last)
{
    ...
    UInt32 dmg = _formula->calcDamage(atk, def, bf->getLevel(), 1.0f, reduce, _formula->calcAttackPierce(bf));
    ...
    if(u > 0)
    {
        ...
        if(bf->getTyslSSAddCnt())
        {
            ...
            for(UInt8 i = 0; i < 25; i++)
            {
                ...
                if(skill)
                    bo->setTyslSSCnt2(bo->getTyslSSCnt2() + 1);
            }
        }
    }
...
bool BattleSimulator::protectDamage(BattleFighter* bf, BattleFighter* pet, float& phyAtk, float& magAtk, float factor)
{
    ...
    if(phyAtk)
    {
        ...
        dmg = _formula->calcDamage(factor * phyAtk, def, bf->getLevel(), toughFactor, atkreduce, _formula->calcAttackPierce(bf));
...
void BattleSimulator::doSneakAttack(BattleFighter* bf, BattleFighter* bo, bool& pr, bool& cs)
{
    ...
    if(!defend100 && (bo_stun > 0 || (!enterEvade && bf->calcHit(bo, NULL) && !bo->getMoEvade100())))
    {
        ...
        float def = getBFDefend(bo);
        float atkreduce = getBFAtkReduce(bo);
        dmg = _formula->calcDamage(atk * sneak_atk, def, bf->getLevel(), toughFactor, atkreduce, _formula->calcAttackPierce(bf));
        dmg *= static_cast<float>(950 + _rnd(100)) / 1000;
        dmg = dmg > 0 ? dmg : 1;
        makeDamage(bo, dmg, e_damNormal, e_damagePhysic);
        UInt32 rhp = (dmg >> 1);
        ...
        if(eft[i] == GData::e_eft_abnormal_type_dmg)
        {
            ...
            for(size_t j = 0; j < cnt2; ++ j)
            {
                ...
                if(!defend100 && (target_stun > 0 || (!enterEvade && bf->calcHit(target, NULL))))
                {
                    ...
                    float dmg = _formula->calcDamage(atk * factor, def, bf->getLevel(), toughFactor, reduce, _formula->calcAttackPierce(bf));
                    dmg *= static_cast<float>(950 + _rnd(100)) / 1000;
                    dmg = dmg > 0 ? dmg : 1;
...
void BattleSimulator::doPassiveSkillBePHYDmg(BattleFighter* bf, BattleFighter* bo, UInt32 dmg)
{
    ...
    float def = getBFDefend(bf);
    float atkreduce = getBFAtkReduce(bf);
    UInt32 dmg2 = _formula->calcDamage(atk, def, bo->getLevel(), 1, atkreduce, _formula->calcAttackPierce(bo));
    dmg2 *= static_cast<float>(950 + _rnd(100)) / 1000;
    dmg2 = dmg2 > 0 ? dmg2 : 1;
...
bool BattleSimulator::doSkillDmg(BattleFighter* bf, const GData::SkillBase* skill)
{
   ...
   for(size_t i = 0; i < cnt; ++ i)
   {
       ...
       if(atk > 0)
       {
           ...
           float def = getBFDefend(bo);
           float atkreduce = getBFAtkReduce(bo);
           dmg2 = _formula->calcDamage(atk, def, bf->getLevel(), 1, atkreduce, _formula->calcAttackPierce(bf));
           dmg2 *= static_cast<float>(950 + _rnd(100)) / 1000;
           dmg2 = dmg2 > 0 ? dmg2 : 1;
       ...
       else if(magatk > 0)
       {
           ...
           dmg2 = _formula->calcDamage(magatk, magdef, bf->getLevel(), 1, magatkreduce, _formula->calcAttackPierce(bf));
           dmg2 *= static_cast<float>(950 + _rnd(100)) / 1000;
           dmg2 = dmg2 > 0 ? dmg2 : 1;
        ...
        for(size_t j = 0; j < cnt2; ++ j)
        {
            ...
            if(!defend100 && (target_stun > 0 || (!enterEvade && bf->calcHit(target, NULL))))
            {
                ...
                else
                {
                    ...
                    float toughFactor = pr2 ? target->getTough(bf) : 1.0f;
                    float factor = atklist[j].factor;
                    dmg = _formula->calcDamage(atk * factor, def, bf->getLevel(), toughFactor, reduce, _formula->calcAttackPierce(bf));
                    dmg *= static_cast<float>(950 + _rnd(100)) / 1000;
                    dmg = dmg > 0 ? dmg : 1;
...
void BattleSimulator::doSSMakeDamage(BattleFighter* bf, BattleFighter* target, float factor)
{
    if(!bf || !target)
        return;
    if(factor < 0.001f)
            ...
            case GData::e_eft_evolution4:
                {
                    ...
                    for(UInt8 i = 0; i < cnt ; ++i)
                    {
                        ...
                        float toughFactor = pr2 ? bo->getTough(bf) : 1.0f;
                        float factor = 1;
                        float dmg = _formula->calcDamage(atk * factor, def, bf->getLevel(), toughFactor, 0, _formula->calcAttackPierce(bf));
                        dmg *= static_cast<float>(950 + _rnd(100)) / 1000;
                        dmg = dmg > 0 ? dmg : 1;
