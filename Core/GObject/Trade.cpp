			...
			for (UInt16 i = 0; i < itemNum; ++i)
			{
				...
				if (IsEquipId(item->getId()))
				{
					...
					if(item->getQuality() >= 4)
						DBLOG().PushUpdateData("insert into `equip_courses`(`server_id`, `player_id`, `template_id`, `equip_id`, `from_to`, `happened_time`) values(%u, %" I64_FMT "u, %u, %u, %u, %u)", cfg.serverLogId, _owner->getId(), item->GetItemType().getId(), item->getId(), ToDeleteTrade, TimeUtil::Now());
				}
				SAFE_DELETE(item);
			}
		}
...
bool Trade::appendTradeItems(ItemBase** items, UInt16 itemNum)
{
	...
	for (UInt16 i = 0; i < itemNum; ++i)
	{
		...
		if (IsEquipId(items[i]->getId()))
        {
            if(IsZhenYuanItem(items[i]->GetTypeId()))
                ++grids4;
            else
			    ++grids;
        }
		else
        {
            ...
            if(GetItemSubClass(items[i]->getId()) == Item_Soul)
                ...
                grids4 += size;
            else if(GetItemSubClass(items[i]->getId()) == Item_SL)
                grids5 += size;
            else
                grids += size;
        }
	}
	if (grids > package->GetRestPackageSize(0) || grids1 > package->GetRestPackageSize(1) || grids3 > package->GetRestPackageSize(3) || grids4 > package->GetRestPackageSize(4) || grids5 > package->GetRestPackageSize(5))
	{
