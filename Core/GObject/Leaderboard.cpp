...
void Leaderboard::sendGoldLvlAward(BinaryReader& brd)
{
    ...
    */
    static const UInt32 goldLvls[] = { 2000, 5000, 8000, 14000, 20000, 30000 };
    static MailPackage::MailItem s_items[][6][5] ={
        ...
        //2
        {
        { {9424, 5},   {503, 5},  {512, 5},   {0, 0}, {0, 0} },
        { {517, 5},  {503, 5}, {9418, 5},  {512, 5},  {551, 5} },
        { {513, 8},  {134, 8},  {16001, 8},   {1126, 8}, {0, 0} },
        { {9414, 15}, {1325, 15}, {0, 0},  {0, 0}, {0, 0} },
        { {9457, 15}, {9498, 15}, {9600, 15}, {16001, 15}, {0, 0} },
        { {9075, 20},  {0, 0}, {0, 0}, {0, 0}, {0, 0} },
        },
        //3
        {
        ...
        { {554, 20}, {16001, 20}, {9438, 20}, {0, 0}, {0, 0} },
        { {9068, 20},  {0, 0}, {0, 0}, {0, 0}, {0, 0} },
        },
        //4
        {
        { {549, 1},   {1126, 5},  {9424, 5},   {0, 0}, {0, 0} },
        { {9310, 5},  {516, 5}, {500, 5},  {503, 5},  {513, 5} },
        { {515, 6},  {551, 6},  {9308, 6},   {9418, 6}, {0, 0} },
        { {501, 30}, {9457, 30}, {16001, 30},  {0, 0}, {0, 0} },
        { {9076, 20}, {0, 0}, {0, 0}, {0, 0}, {0, 0} },
    ...
    if(now < begin)
        return ;
    UInt8 day = (now - begin) / 86400;
    if(day > 9)
        return ;
    //MailPackage::MailItem s_items[6][5] = s_itemss[day] ;
    SYSMSGV(title, 831);
    for(UInt8 i = 0; i < cnt; ++ i)
    {
        UInt8 idx = 0xFF;
        ...
        MailItemsInfo itemsInfo(s_items[day][idx], Activity, 5);
        Mail * mail = player->GetMailBox()->newMail(NULL, 0x21, title, content, 0xFFFE0000, true, &itemsInfo);
        if(mail)
             mailPackageManager.push(mail->id, s_items[day][idx], 5, true);
    }
}
