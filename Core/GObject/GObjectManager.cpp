...
#define LOAD_LINEUP(lus, lud) \
            ...
            if (dbpd.pdata.packSizeLS < 50)
            {
                dbpd.pdata.packSizeLS = 50;
				DB1().PushUpdateData("UPDATE `player` SET `packSizeLS` = %u WHERE `id` = %" I64_FMT "u", dbpd.pdata.packSizeLS, id);
            }
