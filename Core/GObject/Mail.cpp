...
bool MailPackage::takeIt( Player * player, bool gm )
{
	...
	if(required > package->GetRestPackageSize(0) || required1 > package->GetRestPackageSize(1) || required1 > package->GetRestPackageSize(3) || required4 > package->GetRestPackageSize(4))
		return false;
	for(it = _items.begin(); it != _items.end(); ++ it)
	{
		if(it->first >= 0x8000)
