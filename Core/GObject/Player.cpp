...
void Player::getFriendlyAchievement(UInt8 opt)
{
    ...
    for(;it!=_friendlyCount.end();++it)
    {
        if(it->second.value < 100)
            Friends[0]++;
        else if(it->second.value < 500)
            Friends[1]++;
        else if(it->second.value < 2000)
            Friends[2]++;
        else if(it->second.value < 5000)
            Friends[3]++;
        else
            Friends[4]++;
    }
...
void Player::drinking(Player * friendOne, UInt8 drinkCount)
{
    ...
    float drinkingPoint = DrinkingPoint(friendOne);
...
float Player::DrinkingPoint(Player *friendOne)
{
    if(getDrinkInfo().drinker != friendOne)
        return 0;
    float drinkingAdd = 0;
    ...
    float drinkingPoint=(drinkingAdd + friendAdd)*_drinkingSum;
