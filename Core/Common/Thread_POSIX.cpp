...
#	endif
#endif
#if _OS == _OS_LINUX || _OS == _OS_MAC_OS_X || _OS == _OS_QNX
#	include <time.h>
#endif
...
void ThreadImpl::sleepImpl(long milliseconds)
{
	...
	while (remainingTime > 0 && rc < 0 && errno == EINTR);
	if (rc < 0 && remainingTime > 0) throw SystemException("Thread::sleep(): select() failed");
#endif
}
...
int ThreadImpl::mapPrio(int prio)
{
	...
	case PRIO_HIGHEST_IMPL:
		return pmax;
	default:
		common_bugcheck_msg("invalid thread priority");
	}
